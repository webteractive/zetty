...
public enum AgentHookScript {
    ...
    # Only reports sessions hosted INSIDE quertty: quertty sets QUERTTY=1 in its
    # panes' environment, so hooks fired from other terminals stay silent.
    import sys, os, json

    SINK = os.path.expanduser("~/.quertty/agent-events.jsonl")
    IN_QUERTTY = bool(os.environ.get("QUERTTY"))
