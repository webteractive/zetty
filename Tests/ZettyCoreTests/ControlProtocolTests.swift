...
@Test func controlRequestRoundTripsThroughJSONLines() throws {
    ...
    #expect(try ControlWire.decodeRequest(ControlWire.encodeLine(ControlRequest.quit(killSessions: false, simulateRestart: false)))
            == .quit(killSessions: false, simulateRestart: false))
    #expect(try ControlWire.decodeRequest(ControlWire.encodeLine(ControlRequest.quit(killSessions: true, simulateRestart: false)))
            == .quit(killSessions: true, simulateRestart: false))
    #expect(try ControlWire.decodeRequest(ControlWire.encodeLine(ControlRequest.quit(killSessions: false, simulateRestart: true)))
            == .quit(killSessions: false, simulateRestart: true))
    // An older CLI's payload has neither flag.
    #expect(try ControlWire.decodeRequest(#"{"command":"quit"}"#) == .quit(killSessions: false, simulateRestart: false))
